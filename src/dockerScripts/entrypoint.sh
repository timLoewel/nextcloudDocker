...
# contains fixes from https://github.com/nextcloud/docker/pull/588
...
if expr "$1" : "apache" 1>/dev/null || [ "$1" = "php-fpm" ] || [ "${NEXTCLOUD_UPDATE:-0}" -eq 1 ]; then
    ...
    echo "nextcloud image version: $image_version"
        ...
        if [ "$installed_version" = "0.0.0.0" ]; then
            ...
            if [ -n "${NEXTCLOUD_ADMIN_USER+x}" ] && [ -n "${NEXTCLOUD_ADMIN_PASSWORD+x}" ]; then
                ...
                if [ "$install" = true ]; then
                    echo "starting nextcloud installation with options: $install_options"
                    max_retries=10
                    try=0
